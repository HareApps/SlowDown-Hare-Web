# 🐇 SlowDown-Hare-Web

> **Rest your eyes, refresh your mind.**  
> Inteligentny asystent przerw, który dba o Twoją higienę pracy przy komputerze.

SlowDown-Hare-Web to nowoczesna aplikacja webowa (PWA) zaprojektowana w estetyce **Glassmorphism**, która pomaga użytkownikom utrzymać zdrowe nawyki poprzez regularne przypomnienia o przerwach. Powstała medodą vibe-codingu w Google AI Studio. 

---

## ✨ Funkcje

- 🕒 **Elastyczne Interwały**: Wybierz czas pracy: 30m, 1h, 1.5h lub 2h.
- ⏳ **Tryb Odliczania**: Precyzyjne odliczanie oparte na czasie systemowym, odporne na uśpienie karty w przeglądarce.
- 🔔 **Inteligentne Alerty**: Powiadomienia o konieczności zrobienia przerwy z opcją "RELAX NOW" lub "Odłóż".
- 🎵 **Powiadomienia Dźwiękowe**: Kojący sygnał audio przy każdym alercie.
- 🧘 **Sesje Relaksacyjne**: Dedykowany ekran relaksu z zorzą polarną, pomagający odciążyć wzrok.
- 📱 **PWA & Offline**: Możliwość instalacji na telefonie/komputerze i działania bez dostępu do internetu.
- 🎨 **Modern UI**: Interfejs oparty na przezroczystościach, rozmyciach i płynnych animacjach.

## 🛠️ Tech Stack

- **Frontend**: HTML5, Vanilla JavaScript
- **Stylizacja**: Tailwind CSS
- **PWA**: Service Worker & Web Manifest

## Podziękowania: 
- **Amarga** - za obraz hare_relax
- **SoundReality** i **Pixabay** za dźwięk soundreality-notification-piano
- **Gemini** i **Nano banana 2** za zorzę i icon

## 🚀 Uruchomienie

Aplikacja jest gotowa do użycia bezpośrednio w przeglądarce. Dzięki technologii PWA możesz ją dodać do ekranu głównego swojego urządzenia, aby mieć do niej szybki dostęp jak do natywnej aplikacji.

---
*Zadbaj o swoje oczy. Zacznij przerwę z zającem.* 🌿
